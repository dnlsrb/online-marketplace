 ...
 <div class="overflow-x-auto "> 
  @forelse ($cart->cartProducts as $c_product)
        <div class="flex flex-row items-center my-2 border-0 border-b-2 p-2">
          <input id="default-checkbox" type="checkbox" value="" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-400 me-3 focus:ring-blue-500 focus:ring-2">
          {{-- cart image --}}
          <img class="hidden h-10 w-10 dark:block basis-8 me-3" src="{{$c_product->product->image}}" alt="imac image" />
          {{-- cart name --}}
          <div class="basis-2/4">
            <h2 class="hover:underline cursor-pointer text-truncate me-3">{{$c_product->product->name}}</h2> 
          </div>
          {{-- quantity --}}
          <div class="flex items-center basis-1/4 me-3">
            <button type="button" id="decrement-button-{{$c_product->id}}" data-input-counter-decrement="counter-input-{{$c_product->id}}" class=" border-gray-300 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-md     hover:bg-gray-200  ">
                <i class="fa-solid fa-minus"></i>
            </button> 
            <input type="text" id="counter-input-{{$c_product->id}}" value="{{$c_product->quantity}}"   
            ...
            <button type="button" id="increment-button-{{$c_product->id}}" data-input-counter-increment="counter-input-{{$c_product->id}}" class="inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-md     hover:bg-gray-200  ">
            ...
            <div class="basis-8">
              <form action=""> 
              <input type="submit"  value="Remove" class="text-red-500  hover:underline cursor-pointer"> 
            </form>
            </div>
        </div>
        @empty
